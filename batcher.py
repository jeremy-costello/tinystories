...
def create_batches(spark, data_parquet_root, tokenizer_save_location,
                   context_length, tokenized_parquet_root, eos_token):
    ...
    for data_split in ["train", "valid"]:
        df = spark.read.parquet(f"{data_parquet_root}-{data_split}.parquet")
        ...
        df.write.parquet(f"{tokenized_parquet_root}-{data_split}.parquet", mode="overwrite")
